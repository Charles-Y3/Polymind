...
import {updateProfile, type PolymindProfile} from '../profile/profileStore';
...
export default function Hub({profile, onProfileChange}: HubProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_15%_-10%,rgba(34,211,238,0.16),transparent_42%),radial-gradient(circle_at_85%_0%,rgba(245,158,11,0.13),transparent_42%),radial-gradient(circle_at_15%_60%,rgba(139,92,246,0.14),transparent_45%),radial-gradient(circle_at_90%_70%,rgba(217,70,239,0.13),transparent_45%)]" />
        ...
        <section className="grid sm:grid-cols-[1.3fr_1fr] gap-6 mb-8">
          <div className="rounded-3xl border border-slate-800 bg-slate-900/50 p-6 flex flex-col justify-center">
            ...
            <div className="mt-4 flex items-center gap-3">
              <Link
                to="/leaderboard"
                className="inline-flex items-center gap-1.5 text-xs font-semibold text-amber-300 hover:text-amber-200 border border-amber-500/30 hover:border-amber-400/50 rounded-full px-3 py-1.5 transition-colors"
              >
        ...
        <section className="grid sm:grid-cols-2 gap-4">
          {GAMES.map((game) => {
            ...
            return (
              ...
              >
                <div className={`absolute inset-0 opacity-[0.06] group-hover:opacity-15 bg-gradient-to-br ${game.accent.from} ${game.accent.to} transition-opacity`} />
